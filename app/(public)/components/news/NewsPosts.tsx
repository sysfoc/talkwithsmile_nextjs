// app/(public)/components/news/NewsPosts.tsx
"use client";
...
const NewsPosts = () => {
  ...
  const fetchNewsData = async () => {
    try {
      const blogsRes = await fetch("/api/v1/blog/get-news-blogs", {
        method: "GET",
        credentials: "include",
      });
      if (!blogsRes.ok) throw new Error("Failed to load news");
      const blogsData = await blogsRes.json();
      setNewsData(blogsData.blogs);
    } catch (error) {
      console.log(error);
    }
  };
