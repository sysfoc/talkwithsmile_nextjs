// app/api/v1/blog/get-general-blogs/route.ts
...
export async function GET() {
  ...
  try {
    const blogs = await Blog.find({ type: "1" })
      .sort({ createdAt: -1 })
      .select("_id id title slug image description status author views type created_at")
      .lean();
